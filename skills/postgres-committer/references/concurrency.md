...
1. SpinLocks — busy-wait locks that must be held for only a few instructions. No
   `CHECK_FOR_INTERRUPTS()` or blocking operations while held. Will abort after
   ~1 minute if not acquired.
...
deadlocks. Violating lock ordering is a common source of hard-to-reproduce bugs.
